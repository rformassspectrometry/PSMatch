...
vignette: >
    ...
    %\VignettePackage{psm}
    %\VignetteDepends{mzR,mzID,BiocStyle,msdata,magrittr,dplyr,tidyr,msdata}
---
...
`backend` parameter, namely `"mzR"` (which is also the default value)
and `"mzID"`.

- **mzR** uses the `openIDfile()` function from the `r
  BiocStyle::Biocpkg("mzR")` to parse the `mzId` file(s), and then
...
```{r}
system.time(id1 <- readPSMs(f, backend = "mzR"))
system.time(id2 <- readPSMs(f, backend = "mzID"))
```

Here, we see that the variables names are different, and that the
variables returned by the two backend are also different - note for
instance (and this will be important later), that there is not
equivalent of `"modLocation"` in `id2`.
